...
describe('InfraStack', () => {
  ...
  test('creates a public ECS platform service when enabled', () => {
      ...
      ContainerDefinitions: Match.arrayWith([
        Match.objectLike({
          ...
          Secrets: Match.arrayWith([
            ...
            Match.objectLike({
              Name: 'PUDDLE_ASHBY_WEBHOOK_SECRET',
            }),
            Match.objectLike({
              Name: 'PUDDLE_BACKEND_INTERNAL_TOKEN',
            }),
    ...
    expect(taskSecretNames(template, 'puddle-videoagent-platform', 'platform')).toEqual(
      expect.arrayContaining(['PUDDLE_ASHBY_WEBHOOK_SECRET']),
    );
    expect(executionRolePolicyAllowsSecret(template, 'PlatformExecutionRole', 'AshbyWebhookSecret')).toBe(
      true,
    );
  });
});
