...
ENABLE_LIVEKIT_RECORDINGS="${ENABLE_LIVEKIT_RECORDINGS:-${PUDDLE_RECORDINGS_ENABLED:-false}}"
PARTICIPANT_RECONNECT_GRACE_SECONDS="${PARTICIPANT_RECONNECT_GRACE_SECONDS:-${PUDDLE_PARTICIPANT_RECONNECT_GRACE_SECONDS:-300}}"
PLATFORM_ASHBY_ONBOARDING_ADMIN_EMAILS="${PLATFORM_ASHBY_ONBOARDING_ADMIN_EMAILS:-${PUDDLE_ASHBY_ONBOARDING_ADMIN_EMAILS:-}}"
...
if [[ -n "$PLATFORM_ASHBY_ONBOARDING_ADMIN_EMAILS" ]]; then
  CDK_CONTEXT_ARGS+=(
    -c platformAshbyOnboardingAdminEmails="$PLATFORM_ASHBY_ONBOARDING_ADMIN_EMAILS"
  )
fi
