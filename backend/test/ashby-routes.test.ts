...
describe("Ashby backend routes", () => {
  ...
  it("marks ready signed application updates without job ids processed without writing application rows", async () => {
    process.env.PUDDLE_INTEGRATION_SECRET_KEY = "test-secret";
    const rawBody = JSON.stringify({
      webhookActionId: "action_missing_job",
      action: "applicationUpdate",
      data: {
        application: {
          id: "app_missing_job",
          candidate: {
            id: "cand_missing_job",
            name: "Ari Lane",
            primaryEmailAddress: "ari@example.com",
          },
          status: "Active",
  ...
  it("returns imported evaluation candidates when no Ashby jobs are configured", async () => {
    ...
    try {
      ...
      expect(queryMock).toHaveBeenCalledTimes(3);
      expect(queryMock.mock.calls[1]?.[1]).toEqual(["int_1", []]);
      expect(queryMock.mock.calls[2]?.[1]).toEqual(["int_1", [], 200]);
    } finally {
      await app.close();
    }
  });
